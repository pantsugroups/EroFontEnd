<template>
  <div>
    ...
    <div class="ui centered grid container">
      <GameCard v-for="gameobj in games" :key="gameobj.id" :game="gameobj"></GameCard>
      ...
      <div class="sixteen wide column">
        <div class="fluid ui buttons">
          <button :class="[{ disabled: page === 1 }, 'ui', 'button']" @click="prePage">上一页</button>
          <button
            :class="[{ disabled: page === totalPage }, 'ui', 'button']"
            @click="nextPage"
          >{{ page !== totalPage ? "下一页" : "没有更多了" }}</button>
        </div>
      </div>
    </div>
  </div>
